...
function DishModal({ tableId, onClose, tableName }) {
  ...
  const { cartData, addToCart, updateQuantity, removeFromCart, resetCart } =
    useTableCart();
  ...
  const totalAmount = cartData[tableId]?.reduce(
  ...
  return (
    <>
          ...
          <div className="modal-body" onWheel={(e) => e.stopPropagation()}>
              ...
              {filteredFoodList.map((dish) => (
                <div key={dish._id}>
                  <DishItem
                    ...
                    price={dish.price}
                    image={`${url}/images/` + dish.image}
                    onClick={() => addToCart(tableId, dish)}
                  />
                </div>
              ))}
            </div>
            <div className="cart-container">
              <div className="button-component">
                <button className="button" onClick={() => resetCart(tableId)}>
                  Đặt lại
                </button>
                    ...
                    <tbody>
                      {cartData[tableId]?.map((item) => (
                        <tr key={item._id}>
                          ...
                          <td>
                            <button
                              onClick={() => removeFromCart(tableId, item._id)}
                            >
                              X
                            </button>
                          </td>
              ...
              <p className="total-amount">
                Tổng tiền hóa đơn: {formatNumber(totalAmount + "")} vnđ
              </p>
            </div>
          </div>
        </div>
      ...
      {isModalOpen && (
        <InvoiceModal onClose={closeModal} invoiceData={invoiceData} />
      )}
    </>
  );
}
